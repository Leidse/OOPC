...
public:
   ...
   bool operator==( const rational & rhs ) const {
       ...
       int denominator2;
        for (int i = 1; i < 100; i++) {
            if (((counter / i) == rhs.counter) && ((denominator / i) == rhs.denominator)) {
                counter2 = counter / i;
                denominator2 = denominator / i;
            }
        }
        return ( counter2 == rhs.counter ) || ( denominator2 == rhs.denominator );
   }
   ...
   rational & operator+=( const rational & rhs ){
    ...
    /// \details
    /// This operator*= multiplys a rational value a rational variable.
   rational operator*=( const rational & rhs ) {
        return rational(
